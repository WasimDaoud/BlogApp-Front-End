import React, { useState } from "react";
import { BsSendFill } from "react-icons/bs";
import { toast } from "react-toastify";

import { useDispatch } from "react-redux";

import { UpdateTheComment } from "../Redux/apiCalls/CommentApiCalls";
import { commentActions } from "../Redux/Slices.js/CommentSlice";

const UpdateComment = ({ comment , setCommentModal }) => {

 const dispatch = useDispatch();

  const [UpdatedComment, setUpdatedComment] = useState(comment.text);

  const newUpdatedCommentSubmitHandler = () => {
    dispatch(commentActions.setCommentIsUpdated(false));
    if (!UpdatedComment) {
      return toast.error("You Must Make Some changing");
    }
    const text = {
      "text" : UpdatedComment
    } ;
    dispatch(UpdateTheComment( comment?._id , text ));
    toast.success("Comment has been Updated Successfully");
    dispatch(commentActions.setCommentIsUpdated(true));
    setCommentModal(false);
  };

  return (
    <div className="w-[90%] m-auto rounded-xl p-[25px] my-[15px] border-2 border-blue">
      {/* Update comment */}
      <strong className="underline text-[20px] md:text-[25px] text-blue dark:text-blue">
        Update Comment
      </strong>
      {/* new-comment form */}
      <div className="bg-gray dark:text-gray-dark-i dark:bg-gray-dark-bg border-2 dark:border-gray-dark-i rounded-xl my-[20px]">
        <form className="flexBetween px-[15px]">
          {/* input field for Update comment */}
          <input
            className="min-w-[200px] text-[20px]  dark:text-gray-dark-i input py-[10px]"
            placeholder="write comment here"
            value={UpdatedComment}
            onChange={(e) => {
              setUpdatedComment(e.target.value);
            }}
          ></input>
          {/* icon for submitting */}
          <abbr title="Update">
            <BsSendFill
              onClick={newUpdatedCommentSubmitHandler}
              className="md:text-[30px] text-[25px] hover:scale-110 duration-500 cursor-pointer text-blue"
            />
          </abbr>
        </form>
      </div>
      <hr className="text-2 mt-[40px] text-gray dark:text-gray-dark"></hr>
    </div>
  );
};

export default UpdateComment;
